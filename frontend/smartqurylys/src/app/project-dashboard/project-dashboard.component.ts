...
import { ProjectNotesModalComponent } from '../project-notes-modal/project-notes-modal.component';
import { ExecutorsModalComponent } from '../executors-modal/executors-modal.component';
import { ReactivationModalComponent } from './reactivation-modal.component';

@Component({
  selector: 'app-project-dashboard',
  standalone: true,
  imports: [CommonModule, ModalComponent, FormsModule, RouterModule, ChatComponent, ProjectNotesModalComponent, ExecutorsModalComponent, ReactivationModalComponent],
...
export class ProjectDashboardComponent implements OnInit {
  ...
  getViewTitle(): string {
    switch (this.currentView) {
      case 'stages': return 'ЭТАПЫ ПРОЕКТА';
      case 'users': return 'Участники проекта';
  ...
  toggleParticipantDetails(id: number): void {
    this.expandedParticipantId = this.expandedParticipantId === id ? null : id;
  }

  getStagesForParticipant(participantId: number): string[] {
    return Array.from(this.participantStagesMap.get(participantId) || []);
  }

  fetchAllTasksForAllStages(): void {
    if (!this.stages.length) return;

    const taskObservables = this.stages.map(stage =>
    ...
    forkJoin(taskObservables).subscribe(results => {
      this.allTasks = results.flat();
      this.buildParticipantStagesMap();
    });
  }

  buildParticipantStagesMap(): void {
    this.participantStagesMap.clear();
    this.stages.forEach(stage => {
      const stageTasks = this.allTasks.filter(t => t.id && this.isTaskInStage(t, stage.id)); // This check is dummy, actually results from forkJoin are already grouped by stage
    });

    // Actually, let's use the results from forkJoin directly to build the map
  }

  // Refined fetch and build
  loadParticipantsData(): void {
    if (!this.stages.length) return;
      ...
      this.taskService.getTasksByStage(stage.id).pipe(
        tap((tasks: TaskResponse[]) => {
          tasks.forEach((task: TaskResponse) => {
            task.responsiblePersons.forEach((person: ParticipantResponse) => {
              if (!this.participantStagesMap.has(person.id)) {
                this.participantStagesMap.set(person.id, new Set());
              }
              this.participantStagesMap.get(person.id)?.add(stage.name);
            });
          });
        }),
        catchError(() => of([]))
    ...
    forkJoin(taskObservables).subscribe(() => {
      // Map is now populated
    });
  }

  private isTaskInStage(task: TaskResponse, stageId: number): boolean {
    // This is hard since TaskResponse doesn't have stageId usually, but we fetch it by stageId
    return true;
  }
  ...
  ngOnInit(): void {
    ...
    // Читаем начальный вид из queryParams
    this.route.queryParams.subscribe(params => {
      if (params['view']) {
        this.currentView = params['view'];
        // Обновляем активность в navItems
  ...
  handleStageAction(action: string, stage: StageResponse): void {
    ...
    switch (action) {
      ...
      case 'returnStageToActive':
        this.openReactivationModal(stage);
        break;
      default:
  ...
  // This method is now replaced by openReactivationModal + submitReactivation
  ...
  openAddTaskModal(): void {
    ...
    this.editableTaskPriority = false;
  ...
  editTask(task: TaskResponse): void {
    ...
    this.editableTaskPriority = task.isPriority || false;
  ...
  togglePriority(): void {
    if (this.selectedTaskForEdit) {
      this.editableTaskPriority = !this.editableTaskPriority;
    }
  }
  ...
  // ========== EXECUTORS MODAL METHODS ==========

  openExecutorsModal(stage: StageResponse): void {
    this.selectedStageForExecutors = stage;
    this.isExecutorsModalOpen = true;
  }

  closeExecutorsModal(): void {
    this.isExecutorsModalOpen = false;
    this.selectedStageForExecutors = null;
  }

  // ========== STAGE REACTIVATION METHODS ==========

  openReactivationModal(stage: StageResponse): void {
    this.selectedStageForReactivation = stage;
    this.isReactivationModalOpen = true;
  }

  closeReactivationModal(): void {
    this.isReactivationModalOpen = false;
    this.selectedStageForReactivation = null;
  }

  submitReactivation(reason: string): void {
    if (!this.selectedStageForReactivation || !this.projectId) return;

    this.stageService.returnStageToActive(this.projectId, this.selectedStageForReactivation.id, reason).subscribe({
      next: (response) => {
        // Refresh project data to show updated stage status
        this.fetchProjectDetails();
        this.closeReactivationModal();
      },
      error: (err) => {
        console.error('Error reactivating stage:', err);
      }
    });
  }
